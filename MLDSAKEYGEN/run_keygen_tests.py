...
def load_seeds(path: Path, parameter: int) -> list[bytes]:
    try:
    ...
    for line_number, raw_line in enumerate(lines, start=1):
        ...
        if len(fields) != 3 or fields[0] != "KEYGEN":
            raise KeygenTestError(f"line {line_number}: expected KEYGEN <parameter> <seed>")
        try:
        ...
        seed = parse_hex(fields[2], f"line {line_number} seed")
        if len(seed) != 32:
            raise KeygenTestError(f"line {line_number}: seed must contain 32 bytes")
        seeds.append(seed)
    if not seeds:
...
def build_golden(parameter: int, seeds: Sequence[bytes]) -> list[KeygenVector]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="mldsa-keygen-") as directory:
            ...
            for seed in seeds:
                output = subprocess.run(
                    [str(helper), seed.hex()], check=True, capture_output=True,
                ).stdout
...
def run_vectors(
    ...
    for index, vector in enumerate(vectors, start=1):
        command = f"KEYGEN {parameter} {vector.seed.hex().upper()}\n".encode("ascii")
        print(f"[{index}/{len(vectors)}] TX ML-DSA-{parameter} KeyGen")
        ...
        print(f"  PASS PK={len(public_key)} bytes SK={len(secret_key)} bytes{timing}")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run project KD ML-DSA KeyGen vectors over UART."
    )
    parser.add_argument("--parameter", type=int, choices=sorted(SUPPORTED_PARAMETERS), default=44)
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        seeds = load_seeds(command_path, args.parameter)
        vectors = build_golden(args.parameter, seeds)
        ...
        try:
            ...
            serial_port.close()
        print(f"PASS: all ML-DSA-{args.parameter} KeyGen cases matched")
        return 0
