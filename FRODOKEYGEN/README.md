# Frodo keygen independent board tests

This directory contains only official KAT keygen inputs and references for
...
- `keygen_commands_P.txt`: complete newline-terminated UART instructions.
- `pk_ref_P.txt` and `pkh_ref_P.txt`: one expected value per instruction.
...
PK is the complete public key. PKH is its SHAKE hash. Both are checked by default.
...
.venv/bin/python -u UARTHELPER/FRODOKEYGEN/run_keygen_tests.py \
    --parameter 640 --port /dev/ttyUSB2 --results /tmp/frodo-keygen-640
```
...
The runner sends `KEYGEN <parameter> 1 0 ...`, suppressing the complete PK
on UART while retaining PKH and cycle counters. Input/reference files remain
unchanged. Results identify compact mode and the checked PKH field; omit
`--compact` for complete PK/PKH verification. See the shared
...
```sh
sed -n '1p' UARTHELPER/FRODOKEYGEN/keygen_commands_640.txt
sed -n '1p' UARTHELPER/FRODOKEYGEN/pk_ref_640.txt
sed -n '1p' UARTHELPER/FRODOKEYGEN/pkh_ref_640.txt
sed -n '1p' UARTHELPER/FRODOKEYGEN/cases_640.jsonl
```
